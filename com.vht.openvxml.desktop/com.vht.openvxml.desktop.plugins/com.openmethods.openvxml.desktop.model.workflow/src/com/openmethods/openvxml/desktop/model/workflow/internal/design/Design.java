...
public class Design implements IDesign, IDesignComponentListener {
	...
	@Override
	public IDesignConnector createDesignConnector(IDesignElement originElement,
			IDesignElement destinationElement) {
		DesignConnector dc = new DesignConnector((DesignElement) originElement,
				(DesignElement) destinationElement);
		if (originElement == destinationElement) {
			dc.addMidpoint(originElement.getCenterPoint().x + 80,
					originElement.getCenterPoint().y - 40);
			dc.addMidpoint(originElement.getCenterPoint().x + 80,
					originElement.getCenterPoint().y + 40);

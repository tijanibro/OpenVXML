...
public class DesignParser {
	...
	public Design parseDesign(IDesignDocument designDocument,
			org.w3c.dom.Element designElement,
			ElementResolutionVisitor resolutionVisitor,
			boolean followExternalReferences) {
		return parseDesign(designDocument, null, designElement,
				resolutionVisitor, followExternalReferences);
	}

	public synchronized Design parseDesign(IDesignDocument designDocument,
			Design baseDesign, org.w3c.dom.Element designElement,
			ElementResolutionVisitor resolutionVisitor,
			boolean followExternalReferences) {
		...
		try {
			...
			org.w3c.dom.Element modelElement = (org.w3c.dom.Element) modelList
					.item(0);
			...
			if (elementsElementList.getLength() > 0) {
				...
				for (int i = 0; i < elementList.getLength(); i++) {
					...
					properties.setProperty("followExternalReferences",
							Boolean.toString(followExternalReferences));
					...
					element = ElementManager.getInstance().loadElement(
							elementType, elementId, elementName, properties);
					if (element != null) {
						...
						if (configurationList.getLength() > 0) {
							org.w3c.dom.Element configurationElement = (org.w3c.dom.Element) configurationList
									.item(0);
							NodeList managedConfigSectionList = configurationElement
									.getElementsByTagName("managed-config");
							for (int c = 0; c < managedConfigSectionList
									.getLength(); c++) {
								org.w3c.dom.Element configSectionElement = (org.w3c.dom.Element) managedConfigSectionList
										.item(c);
								long t = System.currentTimeMillis();
								ConfigurationManager manager = ConfigurationManagerRegistry
								...
								if (manager != null) {
									System.out
											.println("Loaded configuration manager "
													+ manager.getType()
													+ " in "
							...
							NodeList customElementList = configurationElement
									.getElementsByTagNameNS(
											"http://www.eclipse.org/vtp/namespaces/config",
											"custom-config");
							if (customElementList.getLength() > 0) {
								element.readCustomConfiguration((org.w3c.dom.Element) customElementList
										.item(0));
							}
						}
					}
			...
			NodeList connectorElementList = modelElement
					.getElementsByTagName("connectors");
			if (connectorElementList.getLength() > 0) {
				NodeList connectorList = ((org.w3c.dom.Element) connectorElementList
						.item(0)).getElementsByTagName("connector");
				for (int i = 0; i < connectorList.getLength(); i++) {
					org.w3c.dom.Element connectorElement = (org.w3c.dom.Element) connectorList
							.item(i);
					String id = connectorElement.getAttribute("id");
					String originId = connectorElement.getAttribute("origin");
					String destinationId = connectorElement
							.getAttribute("destination");
					DesignElement origin = elementsById.get(originId);
					if (origin == null && baseDesign != null) {
						origin = (DesignElement) baseDesign
								.getDesignElement(originId);
					}
					DesignElement destination = elementsById.get(destinationId);
					if (destination == null && baseDesign != null) {
						destination = (DesignElement) baseDesign
								.getDesignElement(destinationId);
					}
					if (origin == null || destination == null) {
						throw new RuntimeException("orphaned connector: id="
								+ id + " origin=" + originId + " destination="
								+ destinationId);
					}
					DesignConnector connector = new DesignConnector(id, origin,
							destination);
					NodeList recordList = connectorElement
							.getElementsByTagName("record");
					for (int r = 0; r < recordList.getLength(); r++) {
						org.w3c.dom.Element recordElement = (org.w3c.dom.Element) recordList
								.item(r);
						String sourceName = recordElement
								.getAttribute("sourcename");
						// String destinationName =
						// recordElement.getAttribute("destinationname");
						connector.addConnectionPoint(origin
								.getConnectorRecord(sourceName));
					}
			...
			org.w3c.dom.Element canvasElement = (org.w3c.dom.Element) canvasList
					.item(0);
			int orientation = IDesignConstants.LANDSCAPE;
			if (!canvasElement.getAttribute("orientation").equals("")) {
				orientation = Integer.parseInt(canvasElement
			...
			if (paperSizeId != null && !paperSizeId.equals("")) {
				paperSize = PaperSizeManager.getDefault().getPaperSize(
						paperSizeId);
			}
			if (paperSize == null) {
				paperSize = PaperSizeManager.getDefault().getPaperSize(
			...
			NodeList uiElementList = canvasElement
					.getElementsByTagName("ui-element");
			for (int ue = 0; ue < uiElementList.getLength(); ue++) {
				org.w3c.dom.Element uiElementElement = (org.w3c.dom.Element) uiElementList
						.item(ue);
				...
				Point centerPoint = new Point(Integer.parseInt(xs),
						Integer.parseInt(ys));
			...
			for (int ue = 0; ue < uiConnectorList.getLength(); ue++) {
				org.w3c.dom.Element uiConnectorElement = (org.w3c.dom.Element) uiConnectorList
						.item(ue);
				...
				int ly = 0;
				try {
					segment = Integer.parseInt(uiConnectorElement
							.getAttribute("label-segment"));
					lx = Integer.parseInt(uiConnectorElement
							.getAttribute("label-x"));
					ly = Integer.parseInt(uiConnectorElement
							.getAttribute("label-y"));
				} catch (Exception ex) {
				}
				DesignConnector connector = connectorsById.get(connectorId);
				if (connector != null) {
					connector.getConnectorLabel().setPosition(segment, lx, ly);
					NodeList midPointList = uiConnectorElement
							.getElementsByTagName("mid-point");
					for (int i = 0; i < midPointList.getLength(); i++) {
						org.w3c.dom.Element midPointElement = (org.w3c.dom.Element) midPointList
								.item(i);
						int x = Integer.parseInt(midPointElement
								.getAttribute("x"));
						int y = Integer.parseInt(midPointElement
								.getAttribute("y"));

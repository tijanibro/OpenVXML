...
public class OptionSetConfigurationExporter implements IConfigurationExporter {
	...
	public void exportConfiguration(IFlowElement flowElement,
		...
		config.setDataName(variableName);
		config.setSecured(Boolean.parseBoolean(customConfig
				.getAttribute("secured")));
		List<Element> legacyChoicesElement = XMLUtilities.getElementsByTagName(
				customConfig, "choices", true);
		if (legacyChoicesElement.size() > 0) // legacy conversion
		{
			List<Element> choiceElements = XMLUtilities.getElementsByTagName(
					legacyChoicesElement.get(0), "choice", true);
			for (Element choiceElement : choiceElements) {
				...
				for (int j = 0; j < propertyNames.length; ++j) {
					...
					choiceMedia.setPropertyConfiguration(
							propertyNames[j].substring(prefix.length()),
							property);
				}
				...
				for (int j = 0; j < outputNames.length; ++j) {
					...
					choiceMedia.setOutputConfiguration(
							outputNames[j].substring(prefix.length()), output);
				}
				...
				for (int j = 0; j < inputNames.length; ++j) {
					...
					InputConfiguration input = media
							.getInputConfiguration(inputNames[j]);
					media.setInputConfiguration(inputNames[j], null);
					choiceMedia.setInputConfiguration(
							inputNames[j].substring(prefix.length()), input);
				}
			...
			List<Element> brandOrderElements = XMLUtilities
					.getElementsByTagName(customConfig, "brand-order", true);
			...
			for (Element brandOrder : brandOrderElements) {
				...
				NodeList entryElements = brandOrder.getElementsByTagNameNS(
						brandOrder.getNamespaceURI(), "entry"); //$NON-NLS-1$
				...
				config.setBrandedChoices(brand,
						entries.values().toArray(new String[entries.size()]));
				entries.clear();
			}
		} else {
			List<Element> managedConfigs = XMLUtilities.getElementsByTagName(
					flowElement.getConfiguration(), "managed-config", true);
			System.err
					.println("# of managed configs: " + managedConfigs.size());
			for (Element element : managedConfigs) {
				System.err.println("type: " + element.getAttribute("type"));
				if (!"org.eclipse.vtp.configuration.menuchoice".equals(element
						.getAttribute("type"))) {
					continue;
				}
				Element managedConfig = element;
				List<Element> choicesElements = XMLUtilities
						.getElementsByTagName(managedConfig, "choices", true);
				if (choicesElements.size() == 0) {
					continue;
				}
				List<Element> choiceElements = XMLUtilities
						.getElementsByTagName(choicesElements.get(0), "choice",
								true);
				for (Element choiceElement : choiceElements) {
					String name = choiceElement.getAttribute("name"); //$NON-NLS-1$
					String script = null;
					...
					for (int j = 0; j < outputNames.length; ++j) {
						...
						media.setOutputConfiguration(outputNames[j], null);
						choiceMedia.setOutputConfiguration(
								outputNames[j].substring(prefix.length()),
								output);
					}
					String[] inputNames = media.getInputConfigurationNames();
					for (int j = 0; j < inputNames.length; ++j) {
						...
						media.setInputConfiguration(inputNames[j], null);
						choiceMedia
								.setInputConfiguration(inputNames[j]
										.substring(prefix.length()), input);
					}
					SelectionChoiceConfiguration choice = new SelectionChoiceConfiguration(
							ContentLoadingManager.getInstance(),
							InputLoadingManager.getInstance());
				...
				config.setMediaConfiguration(media);
				List<Element> brandOrderElements = XMLUtilities
						.getElementsByTagName(managedConfig, "brand-order",
								true);
				Map<Integer, String> entries = new TreeMap<Integer, String>();
				for (Element brandOrder : brandOrderElements) {
					String brand = brandOrder.getAttribute("brand"); //$NON-NLS-1$
					...
					for (int j = 0; j < entryElements.getLength(); ++j) {
						Element entryElement = (Element) entryElements.item(j);
						entries.put(
								new Integer(entryElement.getAttribute("spot")), //$NON-NLS-1$
								entryElement.getAttribute("name")); //$NON-NLS-1$
					}
					config.setBrandedChoices(brand,
							entries.values()
		...
		Element configElement = actionElement.getOwnerDocument()
				.createElementNS(
						...
						"interactions:selection-request"); //$NON-NLS-1$

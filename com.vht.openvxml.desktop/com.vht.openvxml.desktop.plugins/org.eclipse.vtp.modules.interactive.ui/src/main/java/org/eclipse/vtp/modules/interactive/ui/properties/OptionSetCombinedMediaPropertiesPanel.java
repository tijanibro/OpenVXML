...
public class OptionSetCombinedMediaPropertiesPanel extends
	...
	private OptionSetInformationProvider info = null;
	 ...
	 * @param element
	 */
	public OptionSetCombinedMediaPropertiesPanel(String name,
			IDesignElement element) {
		super(name, element);
		PrimitiveElement primitiveElement = (PrimitiveElement) element;
		info = (OptionSetInformationProvider) primitiveElement
				.getInformationProvider();
		screensByType = MediaConfigurationScreenManager.getInstance()
				.getIndexedMediaConfigurationScreens(
						primitiveElement.getSubTypeId(), this);
	}
	...
	public void createControls(Composite parent) {
		...
		exitTypeLabel.setText("Variable Name: (Optional)");
			...
			public void verifyText(VerifyEvent e) {
				...
				if ("".equals(currentName)
						|| VariableNameValidator
								.followsVtpNamingRules(currentName)) {
		...
		if ("".equals(variableField.getText())
				|| VariableNameValidator.followsVtpNamingRules(variableField
						.getText())) {
			exitTypeLabel.setForeground(exitTypeLabel.getDisplay()
					.getSystemColor(SWT.COLOR_BLACK));
			variableField.setForeground(variableField.getDisplay()
		...
		if (ISecurableElement.class.isAssignableFrom(info.getClass())) {
			...
			secureElementButton
					.setText("This element may contain sensitive data and should be secured");
			gridData = new GridData(GridData.FILL_HORIZONTAL);
			gridData.horizontalSpan = 2;
			secureElementButton.setLayoutData(gridData);
			secureElementButton.setSelection(((ISecurableElement) info)
					.isSecured());
		}

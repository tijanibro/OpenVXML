...
import com.openmethods.openvxml.desktop.model.webservices.schema.Schema;
...
import com.openmethods.openvxml.desktop.model.webservices.wsdl.soap.SoapBindingOperationElement;
import com.openmethods.openvxml.desktop.model.webservices.wsdl.soap.SoapBody;
import com.openmethods.openvxml.desktop.model.webservices.wsdl.soap.SoapConstants;
import com.openmethods.openvxml.desktop.model.webservices.wsdl.soap.SoapFault;
import com.openmethods.openvxml.desktop.model.webservices.wsdl.soap.SoapHeader;
import com.openmethods.openvxml.desktop.model.webservices.wsdl.soap.SoapHeaderFault;
					...
					if (message == null) {
						addProblem(createProblem(
								"Could not resolve message type for operation element: "
										+ child.getAttribute("message"), child));
						continue;
					}
			...
			for (Element operationItemElement : operationItemElementList) {
				if (operationItemElement.getLocalName().equals("input")) {
					SoapBindingOperationElement bindingOperationElement = parseSoapBindingOperationItem(
							operationItemElement, operation);
	...
	public SoapBindingOperationElement parseSoapBindingOperationItem(
		...
		if (bodyElementList.size() != 1) {
			addProblem(createProblem("Wrong number of soap body elements in "
					...
					+ operation.getName(), operationItemElement));
			return null;
		}

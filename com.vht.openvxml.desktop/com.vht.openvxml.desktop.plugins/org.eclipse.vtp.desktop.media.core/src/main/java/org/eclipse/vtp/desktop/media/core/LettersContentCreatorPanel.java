...
public class LettersContentCreatorPanel extends DynamicContentCreatorPanel {
	Text text = null;

	public LettersContentCreatorPanel() {
		super();
	}
	...
	public Content createContent() {
		LettersContent content = new LettersContent();
	...
	public void setInitialContent(Content content) {
		if (content instanceof LettersContent) {
			LettersContent lettersContent = (LettersContent) content;
			if (lettersContent.getValueType() == LettersContent.STATIC_VALUE) {
				setDynamicSelected(false);
				text.setText(lettersContent.getValue());
			} else {
				setDynamicSelected(true);
				setDynamicSelection(lettersContent.getValue());
			}

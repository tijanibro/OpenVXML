 ...
 * {@link org.eclipse.cdt.internal.ui.cview.OpenFileGroup}, but without explicit
 * dependency on CView. This opens it up for use in the Common Navigator
 * framework.
 * <p>
 * Contributes an "Open" action for the default editor, an "Open With" sub-menu
 * for all applicable editors if one or more files are selected. For all
 * container selections, an "Open In New Window" action is contributed.
 * </p>
 * 
 ...
 */
public class WorkflowNavigatorOpenEditorActionGroup extends
		AbstractWorkflowNavigatorActionGroup {
	...
	public void fillContextMenu(IMenuManager menu) {
		IStructuredSelection elements = (IStructuredSelection) getContext()
				.getSelection();
		...
		for (Object obj : elements.toList()) {
			if (obj instanceof IResource) {
				...
				IResource adapted = (IResource) ((IAdaptable) obj)
	...
	private void fillOpenWithMenu(IMenuManager menu,
		...
		submenu.add(new OpenWithMenu(getViewPart().getSite().getPage(),
				(IFile) element));
	 ...
	 */
	/*
	 * private void addNewWindowAction(IMenuManager menu, IStructuredSelection
	 * selection) {
	 * 
	 * // Only supported if exactly one container (i.e open project or folder)
	 * is selected. if (selection.size() != 1) { return; } Object element=
	 * selection.getFirstElement(); if (element instanceof ICElement) { element=
	 * ((ICElement)element).getResource(); } if (!(element instanceof
	 * IContainer)) { return; } if (element instanceof IProject && !(((IProject)
	 * element).isOpen())) { return; }
	 * 
	 * menu.appendToGroup(ICommonMenuConstants.GROUP_OPEN, new
	 * OpenInNewWindowAction(getViewPart().getSite().getWorkbenchWindow(),
	 * (IContainer) element)); }
	 */
	/*
	 * @see
	...
	public void updateActionBars() {
		IStructuredSelection celements = (IStructuredSelection) getContext()
				.getSelection();

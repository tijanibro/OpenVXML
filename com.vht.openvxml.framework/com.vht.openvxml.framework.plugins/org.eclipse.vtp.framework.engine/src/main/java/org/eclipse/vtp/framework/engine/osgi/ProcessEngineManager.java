...
public final class ProcessEngineManager extends SingletonTracker {
	 ...
	 * @author Lonnie Pryor
	 */
	private final class ProcessEngineInstance implements
		...
		void open() {
			...
			synchronized (this) {
				IExtensionPoint[] extensionPoints = new IExtensionPoint[] {
						actions, configurations, observers, services };
			...
			registration = context.registerService(
					IProcessEngine.class.getName(), processEngine, null);
		}
		 ...
		 * 
		 * @param contributor
		 *            The bundle that contributed the element.
		 * @param element
		 *            The configuration element to load from.
		 * @return A new action descriptor.
		 */
		ActionDescriptor loadAction(Bundle contributor,
				IConfigurationElement element) {
			System.out.println("Loading action class: "
		...
		ServiceDescriptor loadService(Bundle contributor,
			...
			for (int i = 0; i < identifierElements.length; ++i) {
				...
				identifiers[i] = new IdentifierDescriptor(
						identifierElements[i].getAttribute("name"), qualifiers); //$NON-NLS-1$
			}
		 ...
		 * 
		 * @param extension
		 *            The extension to find the point of.
			...
			try {
				...
				synchronized (this) {
					...
					for (IConfigurationElement element : elements) {
						...
						if (actions == point) {
							descriptor = loadAction(contributor, element);
						} else if (configurations == point) {
							descriptor = loadConfiguration(contributor, element);
						} else if (observers == point) {
							descriptor = loadObserver(contributor, element);
						} else if (services == point) {
							descriptor = loadService(contributor, element);
						}
						if (descriptor == null) {
							continue;
						}

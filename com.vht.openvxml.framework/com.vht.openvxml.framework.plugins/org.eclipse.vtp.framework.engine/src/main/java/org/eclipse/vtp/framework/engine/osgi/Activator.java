...
public class Activator extends AbstractReporter implements BundleActivator,
	 ...
	 * @return
	 */
	private String buildReport(int severity, String[] categories,
	...
	protected void doReport(int severity, String[] categories, String message,
		...
		if (type == null || "report".equalsIgnoreCase(type.toString())) { //$NON-NLS-1$
			...
			if (reporters != null) {
				for (Object reporter : reporters) {
					((IReporter) reporter).report(severity, categories,
							message, properties);
				}
			}
		...
		System.out.println(buildReport(severity, categories, message,
				properties));
	}
	...
	public void start(BundleContext context) throws Exception {
		...
		logReader = new ServiceTracker(context,
				LogReaderService.class.getName(), this);
		logReader.open();
		...
		context.registerService(
				...
				new LogLevelCommandListener(), null);
	}
	...
	public class LogLevelCommandListener implements CommandProvider {
		public void _vtplogging(CommandInterpreter ci) {
			...
			if ("getLevel".equalsIgnoreCase(command)) {
				ci.println("Current Log Level: "
						+ REPORT_LEVELS[activeLogLevel - 1]);
				return;
			}

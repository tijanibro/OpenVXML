...
public abstract class Configurable extends Component {
	 ...
	 * @param type
	 *            The type of the configurable component.
	 * @param elements
	...
	protected Configurable(Blueprint blueprint, Class type, Element[] elements)
		...
		for (Iterator i = elementList.iterator(); i.hasNext();) {
			...
			Collection configurationList = blueprint
					.createConfigurations(element);
			for (Iterator j = configurationList.iterator(); j.hasNext();) {
				...
				for (Iterator k = identifierSet.iterator(); k.hasNext();) {
					...
					LinkedList items = (LinkedList) configurationIndex
							.get(identifier);
					if (items == null) {
						configurationIndex.put(identifier,
	 ...
	 * 
	 * @param identifier
	 *            The identifier of the configuration object to return.
	 * @param serviceRegistry
	 *            The service registry that is bound to the configurable
	 *            component.
	 ...
	 * 
	 * @param identifier
	 *            The identifier of the configuration objects to return.
	 * @param serviceRegistry
	 *            The service registry that is bound to the configurable
	 *            component.
	...
	protected Object[] lookupAllConfigurations(String identifier,
		...
		for (Iterator j = items.iterator(); j.hasNext(); ++i) {
			Object item = j.next();
			if (item instanceof Configuration) {
				results[i] = ((Configuration) item)
						.createInstance(serviceRegistry);
			} else {
				results[i] = item;
			}
		}
	 ...
	 * 
	 * @param scope
	 *            The scope of the instance to create.
	 ...
	 * 
	 * @param scope
	 *            The scope to create the registry for.

...
public class Sequence extends Scope {
	...
	public Sequence(Execution execution) throws NullPointerException {
		...
		if (position == null) {
			...
			this.next = execution.session.process.blueprint
					.getEntryPoint(entry);
			context.info("Sequence position is null, starting at entry point: "
					+ entry);
	 ...
	 *            The owner of this sequence.
	 * @param next
	 *            The next executable this sequence runs.
	 */
	...
	public Sequence next() {
		...
		if (colon < 0) {
			...
			context.setAttribute("span.primitive",
	...
	protected Collection getServices(String identifier) {
		return execution.session.process.blueprint
				.getActionServices(identifier);
	}

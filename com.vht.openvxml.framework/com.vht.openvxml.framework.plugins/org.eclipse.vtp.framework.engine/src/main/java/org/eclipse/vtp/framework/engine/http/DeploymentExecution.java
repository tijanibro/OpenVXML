...
public class DeploymentExecution implements IExecutionDescriptor, ILinkFactory,
	...
	public ResultDocument doNext() {
		...
		IReporter reporter = (IReporter) execution
				.lookupService(IReporter.class.getName());
		if (reporter.isReportingEnabled()) {
			Dictionary report = new Hashtable();
			report.put("event", "execution.started");
			reporter.report(IReporter.SEVERITY_INFO, "Execution \"" + id
					+ "\" Started", report);
		}
		try {
			...
			while (true) {
				...
				if (document != null) {
					return new ResultDocument(document,
							!execution.hasNextStep());
				}
			}
		} finally {
			platform = null;
		...
		try {
			if (!"true".equals(session.getAttribute("vtp.ended"))) {
				...
				if (reporter.isReportingEnabled()) {
					...
					reporter.report(IReporter.SEVERITY_INFO, "Session \""
							+ session.getSessionID() + "\" Terminated", report);
				}
			}
			interactionTypeRegistry = (IInteractionTypeRegistry) execution
					.lookupService(IInteractionTypeRegistry.class.getName());
	...
	public void setParameter(String parameterName, String value)
			throws NullPointerException {
		setParameters(parameterName, value == null ? null
				: new String[] { value });
	}
	...
	public Object visitInclude(IncludeCommand includeCommand) {
		...
		for (String targetName : targetNames) {
			doc.setVariableValue(targetName, exportVariable(variableRegistry
					.getVariable(includeCommand.getVariableValue(targetName))));
		}
		String[] outgoing = includeCommand.getOutgoingPaths();
		for (String element : outgoing) {
			String[] names = includeCommand.getOutgoingDataNames(element);
			for (String name : names) {
				doc.setOutgoingDataValue(element, name,
						includeCommand.getOutgoingDataValue(element, name));
			}
		}
		String[] paramNames = includeCommand.getParameterNames();
		for (String paramName : paramNames) {
			doc.setParameterValues(paramName,
					includeCommand.getParameterValues(paramName));
		}
		return doc;
	}
	...
	public Object visitExit(ExitCommand exitCommand) {
		ControllerDocument doc = new ControllerDocument();
		doc.setParameterValues("exit",
				new String[] { exitCommand.getExitValue() });
		String[] vars = exitCommand.getVariableNames();
		for (String var : vars) {
			doc.setVariableValue(var,
					exportVariable(variableRegistry.getVariable(var)));
		}
		return doc;
	}
	...
	private Map exportVariable(IDataObject variable) {
		...
		if (variable instanceof IArrayObject) {
			...
			Object[] elements = new Object[array.getLength().getValue()
					.intValue()];
			...
			for (String field : fields) {
				data.put(field, exportVariable(variable.getField(field)));
			}
		}
		return data;
	}
	...
	private final class Link implements ILink {
		...
		Link(String path, boolean appendMode) {
			if (path.startsWith("http://") || path.startsWith("https://")
					|| path.startsWith("dtmf:")) {
				this.path = path;
				return;
			}
			StringBuffer buffer = new StringBuffer();
			String contextPath = HttpUtils.normalizePath(httpRequest
					.getContextPath());
			if (!contextPath.equals("/")) {
				buffer.append(contextPath);
			}
			String servletPath = HttpUtils.normalizePath(httpRequest
					.getServletPath());
			if (!servletPath.equals("/")) {
				buffer.append(servletPath);
			}
			for (StringTokenizer st = new StringTokenizer(
					HttpUtils.normalizePath(path), "/"); st.hasMoreTokens();) //$NON-NLS-1$
			{
				try {
			...
			if (appendMode) {
				...
				if (mode != null && mode.length > 0) {
					...
					IMapObject ipmap = (IMapObject) variableRegistry
							.getVariable("InitialParameters");
					if (ipmap != null) {
						IStringObject modeVar = (IStringObject) ipmap
								.getEntry("MODE");
						if (modeVar != null) {
							if (modeVar.getValue() != null
									&& !modeVar.getValue().equals("")) {
								setParameters("MODE",
										new String[] { modeVar.getValue() });
							}
						}
					}
				}

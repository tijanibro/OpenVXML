...
public class ProcessEngine implements IProcessEngine {
	...
	public void unregisterAction(ActionDescriptor descriptor)
		...
		unregister(actionRegistry,
				IProcessEngineObserver.COMPONENT_TYPE_ACTION,
				descriptor.getId(), descriptor);
	}
	...
	public void registerObserver(ObserverDescriptor descriptor)
		...
		register(observerRegistry,
				IProcessEngineObserver.COMPONENT_TYPE_OBSERVER,
				descriptor.getId(), descriptor);
	}
	...
	public void registerService(ServiceDescriptor descriptor)
		...
		register(serviceRegistry,
				IProcessEngineObserver.COMPONENT_TYPE_SERVICE,
				descriptor.getId(), descriptor);
	}
	 ...
	 *            The identifier of the descriptor.
	 * @param descriptor
	 *            The descriptor object to remove from the registry.
	 */
	...
	private void fireEvent(int eventType, int componentType,
		...
		for (IProcessEngineObserver element : array) {
			if (observers.contains(element)) {
				element.processEngineUpdated(eventType, componentType,
						componentIdentifier);
			}
		}
	}
	...
	/**
	 * Exports the selected descriptors for each identifier to the supplied
	 * array.
	 * 
	 * @param registry
	 *            The registry to export from.
	 * @param results
	 *            The results array to populate.
	 */
	...
	public IProcess createProcess(IProcessDefinition definition,
		...
		return new Process(new Blueprint(definition, configurations, actions,
				observers, services, extensionRegistry), descriptor);
	}

...
public class Deployment implements IProcessDescriptor,
	 ...
	 * @param definition
	 *            The process definition to build from.
	 * @param properties
	 *            The process properties.
	 * @param contributor
	 *            The process contributor.
	...
	public Deployment(IProcessEngine engine, IProcessDefinition definition,
		...
		if (resourceManagerIDs != null) {
			for (String resourceManagerID : resourceManagerIDs) {
				resources.put(resourceManagerID, null);
			}
		}
		if (reporter.isReportingEnabled()) {
			Dictionary report = new Hashtable();
			report.put("event", "process.started");
			reporter.report(IReporter.SEVERITY_INFO, "Process \"" + getID()
					+ "\" Started", report);
		}
	}
	...
	public IDocument start(HttpSession httpSession,
			...
			String prefix, int depth, Map<Object, Object> variableValues,
			Map parameterValues, String entryName, String brand,
			boolean subdialog) {
			...
			synchronized (this) {
				if (unregisterSessionListener == null) {
					try {
						...
						manager.getMethod("addHttpSessionListener",
								HttpSessionListener.class).invoke(null, this);
						unregisterSessionListener = manager.getMethod(
		...
		try {
			result = session.start(httpSession, httpRequest, httpReesponse,
					prefix, depth, variableValues, parameterValues, entryName,
					brand, subdialog);
		} finally {
			session.unlock();
		}
		...
		if (result == null
				|| (result.getDocument() == null && result.isTerminated())) {
			document = abort(httpSession, httpRequest, httpReesponse, prefix,
					depth, variableValues, parameterValues);
		}
	...
	public IDocument next(HttpSession httpSession,
		...
		IDocument document = null;
		if (result == null) {
			document = abort(httpSession, httpRequest, httpResponse, prefix,
					depth, variableValues, parameterValues);
		}
		if (result != null && result.getDocument() != null) {
			document = result.getDocument();
	...
	public IDocument abort(HttpSession httpSession,
		...
		session.lock();
		try {
			return session.abort(httpSession, httpRequest, httpReesponse,
					prefix, depth, variableValues, parameterValues);
		} finally {
			session.unlock();
			synchronized (sessions) {
	...
	public void dispose() {
		...
		if (reporter.isReportingEnabled()) {
			...
			((IReporter) process.lookupService(IReporter.class.getName()))
					.report(IReporter.SEVERITY_INFO, "Process \"" + getID()
							+ "\" Stopped", report);
		}
	}
	...
	public void sessionDestroyed(HttpSessionEvent se) {
		synchronized (sessions) {
			DeploymentSession session = sessions
					.remove(se.getSession().getId());
			if (session != null) {

...
public class IncludeAction implements IAction {
	...
	public IActionResult execute() {
		final String cmd = context.getParameter(context.getActionID());
		if ("complete".equals(cmd)) //$NON-NLS-1$
		{
			...
			if (result == null) {
				result = IActionResult.RESULT_NAME_DEFAULT;
			}
		...
		try {
			...
			final VariableMappingConfiguration platformMapping = new VariableMappingConfiguration();

...
import org.eclipse.vtp.modules.interactive.ui.SubmitInformationProvider.SubmitParameter;
...
public class SubmitPropertiesPanel extends DesignElementPropertiesPanel
{
	...
	IBrand currentBrand;
	/** A list of all SubmitParameter objects configured in this Submit module*/
	List<SubmitParameter> urlParameters = new ArrayList<SubmitParameter>();
	...
	/** A UI table of all SubmitParameter objects configured in this Submit module*/
...
outer:	for(Variable v : vars)
	...
	public void createControls(Composite parent)
	{
		...
		urlParamTable.addDoubleClickListener(new IDoubleClickListener(){
			...
			public void doubleClick(DoubleClickEvent event)
			{
				if(!urlParamTable.getSelection().isEmpty())
				{
					...
					SubmitParameter si = (SubmitParameter)((IStructuredSelection)urlParamTable.getSelection()).getFirstElement();
					vd.setValue(si);
					if(vd.open() == SWT.OK)
					{
		...
		addButton.addSelectionListener(new SelectionListener(){
			...
			{
				ParameterValueDialog vd = new ParameterValueDialog(Display.getCurrent().getActiveShell());
				SubmitParameter si = info.new SubmitParameter("", 0, "");
				vd.setValue(si);
				if(vd.open() == SWT.OK)
				{
	...
	public class ParamTableLabelProvider implements ITableLabelProvider
	{
		...
		{
			SubmitParameter si = (SubmitParameter)element;
			if(columnIndex == 0)
				return si.name;
			return si.value;
		}
...
//						}
	...
	/**
	 * A dialog used to configure the properties of a SubmitParameter object.
	 */
	public class ParameterValueDialog extends FramedDialog
	{
		...
		/** A text field used to display/modify a static value for the variable */
		Text staticValueField;
		/** The SubmitParameter object this dialog will modify*/
		SubmitInformationProvider.SubmitParameter value;
		Color darkBlue;
		Color lightBlue;
		List<ObjectDefinition> vars;
		Composite staticValueComp;
		...
		/**
		 * Specifies which SubmitParameter object to modify
		 * @param value - the SubmitParameter object to modify
		 */
		public void setValue(SubmitInformationProvider.SubmitParameter value)
		{
			this.value = value;
		}

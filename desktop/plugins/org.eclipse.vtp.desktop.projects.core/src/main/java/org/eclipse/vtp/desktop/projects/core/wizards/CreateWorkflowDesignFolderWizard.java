 ...
 * @author Trip
 */
public class CreateWorkflowDesignFolderWizard extends Wizard implements INewWizard,
IExecutableExtension
{
	...
	{
		IDesignFolder bo = designItemContainer.createDesignFolder(bwp.folderNameField.getText());
		if(bo != null)
		{
	...
	private class DesignFolderWizardPage extends WizardPage
	{
		...
		{
			super("DesignFolderPage",
				"Enter a name for the new design folder", null);
		}
		...
		public void createControl(Composite parent)
		{
			...
			folderNameField.addVerifyListener(new VerifyListener()
			{
				...
				public void verifyText(VerifyEvent e)
                {
					...
					String currentName = folderNameField.getText().substring(0, e.start) + e.text + folderNameField.getText(e.end, (folderNameField.getText().length() - 1));
			...
			folderNameField.addModifyListener(new ModifyListener()
				{
					public void modifyText(ModifyEvent e)
					{
						...
						for(IWorkflowResource wr : designItemContainer.getChildren())
						{
							...
							{
								setErrorMessage(
									"A design filder already exists with that name.");
								setPageComplete(false);

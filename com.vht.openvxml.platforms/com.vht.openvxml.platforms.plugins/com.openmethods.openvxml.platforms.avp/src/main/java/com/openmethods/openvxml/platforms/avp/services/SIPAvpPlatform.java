...
public class SIPAvpPlatform extends VoicePlatform {
	...
	private void addSipHeadersToForm(Form form) throws NullPointerException {
		if (variables.getVariable("avayaSipHeaders") != null) {
			Map<String, IDataObject> sipHeaders = ((IMapObject) variables
					.getVariable("avayaSipHeaders")).getValues();
			int headerIndex = 0;
			for (Entry<String, IDataObject> headerEntry : sipHeaders.entrySet()) {
				System.out.print("Adding SIP Header - " + headerEntry.getKey()
						+ ":" + String.valueOf(headerEntry.getValue()));
				form.setProperty(
						"AVAYA_SIPHEADER.session.connection.protocol.sip.unknownhdr["
								+ headerIndex + "].name", headerEntry.getKey());
				form.setProperty(
						"AVAYA_SIPHEADER.session.connection.protocol.sip.unknownhdr["
								+ headerIndex + "].value",
						String.valueOf(headerEntry.getValue()));
